...
def _meshes_from_primitive(geom):
    """Build trimesh list from urdfpy Geometry when .meshes fails (e.g. Cylinder with _meshes=None in pip urdfpy)."""
    inner = getattr(geom, "geometry", None)
    if inner is None:
        return []
    t = type(inner).__name__
    if t == "Cylinder":
        return [tm.creation.cylinder(radius=inner.radius, height=inner.length)]
    if t == "Sphere":
        return [tm.creation.icosphere(subdivisions=1, radius=inner.radius)]
    if t == "Box":
        return [tm.creation.box(extents=inner.size)]
    return []


def _safe_visual_trimesh_items(urdf_asset):
    """Like urdf_asset.visual_trimesh_fk().items() but works when geometry uses primitives (Cylinder/Box/Sphere) with buggy urdfpy _meshes."""
    lfk = urdf_asset.link_fk()
    items = []
    for link in lfk:
        pose_base = lfk[link]
        for visual in link.visuals:
            ...
            try:
                ...
                meshes = _meshes_from_primitive(geom)
            if not meshes:
                continue
            pose = np.dot(pose_base, visual.origin)
            if getattr(geom, "mesh", None) is not None and getattr(geom.mesh, "scale", None) is not None:
                S = np.eye(4)
                S[:3, :3] = np.diag(geom.mesh.scale)
                pose = np.dot(pose, S)
            for mesh in meshes:
                items.append((mesh, pose))
    return items
...
class WarpAsset(BaseAsset):
    ...
    def load_from_file(self, asset_file):
        ...
        try:
            ...
            # pip urdfpy: Cylinder/Sphere have _meshes=None and .meshes does len(self._meshes)
            self.visual_mesh_items = _safe_visual_trimesh_items(self.urdf_asset)
