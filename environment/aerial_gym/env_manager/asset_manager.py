...
class AssetManager:
    def __init__(self, global_tensor_dict, num_keep_in_env):
        ...
        self._logged_obstacle_clamp = False  # only log once when clamping
    ...
    def reset_idx(self, env_ids, num_obstacles_per_env=0):
        if num_obstacles_per_env < self.num_keep_in_env:
            if not getattr(self, "_logged_obstacle_clamp", False):
                self._logged_obstacle_clamp = True
                logger.info(
                    "Number of obstacles required by the task (%s) is less than the env minimum (%s); using minimum (env assets with keep_in_env=True)."
